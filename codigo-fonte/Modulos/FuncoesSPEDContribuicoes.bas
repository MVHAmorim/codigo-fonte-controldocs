...
Public Function ImportarSPEDContribuicoes_Old(Optional ByVal SelReg As String, Optional Periodo As String, Optional ByVal Unificar As Boolean)
...
Dim Arqs As Variant, Arq, Registros, Registro, Campos
Dim dicArquivosPorPeriodo As Dictionary
Dim Chave As Variant, PeriodoArq
Dim arrDados As New ArrayList
Dim nReg As String, versao$
...
Dim Comeco As Double
    ...
    ' ETAPA 1: Chama a nova função para agrupar os arquivos por período
    '--------------------------------------------------------------------------
    Set dicArquivosPorPeriodo = AgruparArquivosPorPeriodo(Arqs) ' -> Use o nome que você definiu para a função aqui (ex: AgruparArquivosSPEDPorPeriodo)
    
    If dicArquivosPorPeriodo.Count = 0 Then
        Call Util.MsgAlerta("Nenhum arquivo SPED Contribuições válido foi selecionado.", "Importação Cancelada")
        Call Util.HabilitarControles
        Exit Function
    End If
    
    ' ETAPA 2: Processar os arquivos, período por período
    '--------------------------------------------------------------------------
    Comeco = Timer
    For Each PeriodoArq In dicArquivosPorPeriodo.Keys
    
        ' Limpa as estruturas de dados para cada novo período
        Call CarregarNiveisRegistros
        Call dicChavesRegistroSPED.RemoveAll
        Call dicRegistros.RemoveAll
        
        a = 0
        For Each Arq In dicArquivosPorPeriodo(PeriodoArq)
            
            If a = 0 Then
                Call fnSPED.ClassificarSPED(Arq, versao, Periodo)
                Call CarregarLayoutSPEDContribuicoes(versao)
                VersaoFiscal = EnumFiscal.ValidarEnumeracao_COD_VER(Periodo)
                Call CarregarLayoutSPEDFiscal(VersaoFiscal)
            End If
            ...
            Application.StatusBar = "Importando Período " & PeriodoArq & " - Arquivo " & (a + 1) & " de " & dicArquivosPorPeriodo(PeriodoArq).Count
            Registros = Util.ImportarTxt(Arq)
            ...
            For Each Registro In Registros
                If VBA.Trim(Registro) <> "" And Registro Like "*|*" And VBA.Len(Registro) > 6 Then
                    ...
                    If nReg = "0000_Contr" And Unificar Then
                        Campos(9) = Util.GerarCNPJCompleto(VBA.Left(Campos(9), 8), "0001")
                        Registro = VBA.Join(Campos, "|")
                    End If
        ...
        Set dicRegistros = fnSPED.DesduplicarRegistros(dicRegistros)
...
Private Function AgruparArquivosPorPeriodo(ByVal Arqs As Variant) As Dictionary

    Dim dicArquivos As New Dictionary
    Dim Arq As Variant
    Dim versao As String, Periodo As String
    
    Application.StatusBar = "Analisando e agrupando arquivos por período, por favor aguarde..."
    
    For Each Arq In Arqs
        If fnSPED.ClassificarSPED(Arq, versao, Periodo) = "Contribuições" Then
            If Not dicArquivos.Exists(Periodo) Then
                Set dicArquivos(Periodo) = New Collection
            End If
            dicArquivos(Periodo).Add Arq
        Else
            Call Util.MsgAlerta("O arquivo '" & Mid(Arq, InStrRev(Arq, "\") + 1) & "' não é um SPED Contribuições válido e será ignorado.", "Verificação Tipo SPED")
        End If
    Next Arq
    
    ' --> Esta é a linha crucial que deve existir para a função retornar o resultado
    Set AgruparArquivosPorPeriodo = dicArquivos
